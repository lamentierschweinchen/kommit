...
export function SignInModal({
  ...
  const handleSubmit = (method: "email" | "google" | "touch_id") => {
    signIn("lukas");
    onOpenChange(false);
    router.push("/dashboard");
    // Slight delay so the toast appears after the modal close animation
    setTimeout(() => confirm("Welcome back."), 220);
    void method;
  };
    ...
    <Modal open={open} onOpenChange={onOpenChange} title="Sign in to back a team">
      ...
      <div className="mt-8 pt-6 border-t-[3px] border-black">
        <p className="text-sm text-gray-500 leading-relaxed">
          We&rsquo;ll create an account for you. Your money stays yours.
        </p>
      </div>
    </Modal>
  );
